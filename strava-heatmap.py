...
def generateHeatmapJS(activityList):
    ...
    for activity in activityList:
        for waypoint in activity['waypoints']:
            js += f'[{waypoint["lat"]}, {waypoint["lon"]}, 0.1],'
    js = js[:-1]
    js += '], {radius: 25}).addTo(map)';
    return js
